...
function babyCareStats(s){
  ...
  const milkOz=milk.reduce((n,e)=>n+(Number(e.amountOz??e.amount_oz)||0),0);
  ...
  return{recent,milkCount:milk.length,milkOz,nursingCount:nursing.length,formulaCount:formula.length,wet,poop,both,diaperCount:diapers.length,lastMilkOz:Number(lastMilk?.amountOz??lastMilk?.amount_oz)||null};
}
...
@media(max-width:760px){#view{transition:none!important}.mf-core-plan{border-radius:24px 34px 25px 31px / 28px 23px 34px 26px}.mf-profile-photo{width:58px;height:58px}.mf-profile-copy h2{font-size:25px}.mf-feed-zone{grid-template-columns:minmax(0,1.5fr) minmax(98px,.78fr)}.mf-feed-main{min-height:136px;padding:20px 17px}.mf-feed-main strong{font-size:20px}.mf-diaper-blob{min-height:101px}.mf-care-ribbon button{padding:8px 11px}}
@media(max-width:380px){.mf-feed-zone{grid-template-columns:1fr}.mf-feed-side{grid-template-columns:1fr 1fr}.mf-feed-side button{min-height:62px}.mf-diaper-cluster{gap:6px}.mf-diaper-blob{min-height:96px;padding-left:6px;padding-right:6px}}
`;
...
function renderMom(s){
  ...
  let prof=document.getElementById('mfMomProfileLine');if(!prof){prof=document.createElement('div');prof.id='mfMomProfileLine';prof.className='mf-profile-line';const copy=hero.querySelector('.hero-copy')||hero;copy.prepend(prof);}
...
function renderBaby(s){
  ...
  const st=babyCareStats(s),g=positiveGreeting(),babyName=s.baby?.name||'Baby',photo=s.baby?.photo||'',age=ageLabel(s.baby?.birthDate);
  ...
  const feedToday=st.milkCount+st.nursingCount+st.formulaCount;
  box.innerHTML=`
    <div class="mf-profile-line"><button type="button" class="mf-profile-photo addable" data-photo aria-label="${photo?'Change Baby photo':'Add Baby photo'}">${profilePhoto(photo,'baby')}</button><div class="mf-profile-copy"><div class="welcome"><b>${g.mark}</b><span>${g.text}</span></div><h2>${esc(babyName)}</h2><small>${age?esc(age)+' · ':''}${photo?'Tap photo to change it':'Tap the photo spot to add one'}</small></div></div>
    ...
    <div class="mf-baby-dayline"><strong>Today</strong> · ${st.milkCount} breast milk ${st.milkCount===1?'feed':'feeds'}${st.milkOz?` · ${+st.milkOz.toFixed(1)} oz`:''} · ${st.nursingCount} nursing · ${st.diaperCount} diapers</div>`;
}
