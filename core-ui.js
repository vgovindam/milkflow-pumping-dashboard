...
function lastFeedText(e){
  if(!e)return{main:'No feed logged yet',sub:'Tap a feed option below'};
  const ago=relativeAgo(e.date,e.time)||to12(mins(e.time));
  if(e.eventType==='nursing'){
    ...
    return{main:dur?`Nursed ${dur} min`:'Nursed',sub:`${ago}${side}`};
  }
  ...
  return{main:`${oz?oz.toFixed(1):'—'} oz ${type}`,sub:ago};
}
...
.mf-last-feed-band{display:grid;grid-template-columns:34px minmax(0,1fr) auto;gap:10px;align-items:center;margin:-2px 0 1px;padding:11px 13px;border-radius:18px;background:linear-gradient(135deg,#e8f6ff,#eef7ff 55%,#eef9f5);border:1px solid #d5e9f0;color:#38536f}.mf-last-feed-band .mf-last-feed-icon{width:34px;height:34px;border-radius:12px;background:rgba(255,255,255,.72);display:grid;place-items:center}.mf-last-feed-band svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.mf-last-feed-band span{min-width:0}.mf-last-feed-band strong{display:block;font-size:13.5px;line-height:1.15;font-weight:850;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.mf-last-feed-band small{display:block;margin-top:3px;font-size:11px;line-height:1.2;color:var(--muted);font-weight:750}.mf-last-feed-band>em{font-style:normal;font-size:10px;font-weight:850;color:var(--baby-ink);background:rgba(255,255,255,.72);padding:6px 8px;border-radius:999px;white-space:nowrap}
...
function renderBaby(s){
  ...
  box.innerHTML=`
    ...
    <div class="mf-last-feed-band" aria-label="Last feeding"><span class="mf-last-feed-icon">${icon('bottle')}</span><span><strong>${esc(lf.main)}</strong><small>${esc(lf.sub)}</small></span><em>Last fed</em></div>
