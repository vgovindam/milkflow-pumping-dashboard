...
function lastFeedText(e){
  ...
  const when=`${e.date===today()?'Today':e.date} · ${to12(mins(e.time))}`;
  if(e.eventType==='nursing'){
    ...
    return{main:dur?`Nursed ${dur} min`:'Nursed',sub:`${when}${side}`};
  }
  ...
  return{main:`${oz?oz.toFixed(1):'—'} oz ${type}`,sub:when};
}
...
.mf-mom-orbs{display:flex;gap:9px;align-items:flex-start;margin:3px 0 9px;padding-left:1px}.mf-mom-orbs .mf-top-orb-wrap{width:58px}.mf-mom-orbs .mf-top-orb{width:54px;height:54px}.mf-mom-orbs .mf-top-orb strong{font-size:14px}.mf-mom-orbs .mf-top-orb-wrap>span{font-size:9px}
...
function renderMom(s){
  ...
  let prof=document.getElementById('mfMomProfileLine');if(!prof){prof=document.createElement('div');prof.id='mfMomProfileLine';const copy=hero.querySelector('.hero-copy')||hero;copy.prepend(prof);}prof.className='mf-profile-line';
  prof.innerHTML=`<button type="button" class="mf-profile-photo addable" data-core-mom-photo aria-label="${photo?'Change Mom photo':'Add Mom photo'}">${profilePhoto(photo,'mom')}</button><div class="mf-profile-copy"><div class="welcome"><b>${g.mark}</b><span>${g.text}</span></div><h2>${esc(name)}</h2>${photo?'':`<small class="mf-photo-hint">Tap the photo spot to add yours</small>`}</div>`;
