...
const THEME_MANIFEST={
  safari:{title:'Safari Adventure',subtitle:'Wild days, bigger dreams',preview:'./assets/themes/safari-world.svg',babyScene:'./assets/themes/safari-world.svg',momPortal:'./assets/themes/safari-adventure.svg'},
  butterfly:{title:'Butterfly Garden',subtitle:'Little moments, big magic',preview:'./assets/themes/butterfly-garden.svg',babyScene:'./assets/themes/butterfly-garden.svg',momPortal:'./assets/themes/butterfly-garden.svg'},
  princess:{title:'Princess Palace',subtitle:'Kind hearts change the world',preview:'./assets/themes/princess-palace.svg',babyScene:'./assets/themes/princess-palace.svg',momPortal:'./assets/themes/princess-palace.svg'},
  unicorn:{title:'Unicorn Dreams',subtitle:'Believe in brighter tomorrows',preview:'./assets/themes/unicorn-dreams.svg',babyScene:'./assets/themes/unicorn-dreams.svg',momPortal:'./assets/themes/unicorn-dreams.svg'},
  clean:{title:'Clean',subtitle:'Quiet MilkFlow canvas',preview:'',babyScene:'',momPortal:''}
};
...
function apply(name=read()){
  ...
  /* These inline custom properties are intentional theme tokens, not element styling.
     They guarantee that a theme switch changes the same artwork consumed by the page,
     home hero and secondary-header components in one render cycle. */
  root.style.setProperty('--mf-theme-art',assetUrl(theme.babyScene));
...
function settingsPanel(){
  ...
  if(!panel){
    panel=document.createElement('section');panel.id='mfExperiencePanel';panel.className='panel mf-experience-panel';
    panel.innerHTML=`<div class="mf-experience-intro"><div><h3>Choose your family world</h3><p>The artwork you choose is the artwork you see on the real Mom and Baby pages — not just a settings preview.</p></div><span class="mf-experience-scope">Mom + Baby</span></div><div class="mf-experience-options" role="group" aria-label="Experience theme">${themeCard('safari','Safari Adventure','Wild days, bigger dreams','./assets/themes/safari-world.svg')}${themeCard('butterfly','Butterfly Garden','Little moments, big magic','./assets/themes/butterfly-garden.svg')}${themeCard('princess','Princess Palace','Kind hearts change the world','./assets/themes/princess-palace.svg')}${themeCard('unicorn','Unicorn Dreams','Believe in brighter tomorrows','./assets/themes/unicorn-dreams.svg')}</div><button type="button" class="mf-experience-clean" data-experience-theme-pick="clean" aria-pressed="false"><span>Clean</span><small>Use the restrained MilkFlow canvas without illustrated scenery</small></button>`;
    const firstPanel=view.querySelector('.panel');if(firstPanel)firstPanel.insertAdjacentElement('beforebegin',panel);else view.appendChild(panel);
  }
  apply();
}
