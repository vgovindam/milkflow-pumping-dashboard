...
if(!chrome){
  console.error('Browser visual audit failed: Chrome/Chromium not available');
  process.exitCode=1;server.close();process.exit();
}
...
await cdp('Page.navigate',{url:'http://127.0.0.1:4173/?visual-audit=1'});
await sleep(2400);
...
async function clickRoute(route){
  return evalJs(`(()=>{const target=${JSON.stringify(route)};const a=[...document.querySelectorAll('[data-view]')].find(x=>x.dataset.view===target);if(!a)return false;a.click();return true})()`);
}
async function screen(){return evalJs('document.body.dataset.screen||""');}
async function reach(route){
  if(await clickRoute(route)){await sleep(180);if(await screen()===route)return true;}
  if(await clickRoute('more')){await sleep(180);if(await clickRoute(route)){await sleep(180);if(await screen()===route)return true;}}
  if(await clickRoute('settings')){await sleep(180);if(route==='settings'&&await screen()==='settings')return true;if(await clickRoute(route)){await sleep(180);if(await screen()===route)return true;}}
  return await screen()===route;
}
...
for(const theme of ['storybook','clean']){
  ...
  for(const route of routes){
    ...
    for(const mode of ['light','dark']){
      await evalJs(`document.documentElement.dataset.theme=${JSON.stringify(mode)};document.querySelector('.main')?.scrollTo(0,0)`);
      await sleep(100);
      const metrics=await evalJs(`(()=>{
        ...
        const animals=[...document.querySelectorAll('.mf-animal-sticker')].map(x=>({art:x.classList.contains('has-storybook-art'),svg:!!x.querySelector(':scope>svg'),img:!!x.querySelector(':scope>img.mf-storybook-animal'),w:x.getBoundingClientRect().width}));
        const visibleText=[...document.querySelectorAll('#view h1,#view h2,#view h3,#view strong')].filter(x=>{const r=x.getBoundingClientRect(),s=getComputedStyle(x);return r.width>0&&r.height>0&&s.visibility!=='hidden'&&s.display!=='none'}).length;
        return{screen:document.body.dataset.screen||'',realm:document.body.dataset.realm||'',viewChildren:v?.children.length||0,viewHeight:vr?.height||0,viewWidth:vr?.width||0,mainWidth:main?.clientWidth||0,scrollWidth:main?.scrollWidth||0,navVisible:!!nr&&nr.width>250&&nr.bottom<=innerHeight+2&&nr.top<innerHeight,navIcons:icons,visibleText,animals};
      })()`);
