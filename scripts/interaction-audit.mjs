...
for(const [theme,baseFile] of Object.entries(themeAssets)){
  need('self-contained scene css',experienceCss,`assets/themes/${baseFile}`);
  need('self-contained scene manifest',experienceJs,`assets/themes/${baseFile}`);
  need('theme detail css',experienceCss,`theme-details/${theme}.svg`);
  need('theme detail manifest',experienceJs,`theme-details/${theme}.svg`);
...
if(experienceJs.includes('theme-composite/'))failures.push('experience controller must not use nested-resource composite SVGs');
...
console.log(`Interaction audit passed: ${routes.length} routes, ${selectors.length} delegated action families, self-contained base scenes + detail layers, independent themed components, five entry forms, navigation recovery, notification identity, and data-preservation contracts.`);
