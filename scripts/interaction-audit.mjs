...
for(const text of ['--mf-icon-sprite','--mf-theme-detail','#view::before','.mf-feed-card::after','.mf-diaper-blob::after','.mf-dream-actions .quick-tile','.mf-settings-theme-panel','.mf-settings-shortcuts','.mf-settings-motto'])need('independent themed components',experienceComponents,text);
for(const theme of ['safari','butterfly','princess','unicorn']){need('theme detail manifest',experienceJs,`theme-details/${theme}.svg`);need('theme icon manifest',experienceJs,`theme-icons/${theme}.svg`);}
for(const scene of ['safari-world.svg','butterfly-garden.svg','princess-palace.svg','unicorn-dreams.svg'])need('renderable theme scene manifest',experienceJs,scene);
...
console.log(`Interaction audit passed: ${routes.length} routes, ${selectors.length} delegated action families, renderable theme scenes, independent themed components, five entry forms, navigation recovery, notification identity, and data-preservation contracts.`);
