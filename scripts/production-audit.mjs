...
for(const [theme,baseFile] of Object.entries(themeAssets))for(const rel of [`assets/themes/${baseFile}`,`assets/theme-icons/${theme}.svg`,`assets/theme-details/${theme}.svg`])if(!fs.existsSync(path.join(DIST,rel)))fail.push(`missing dist/${rel}`);
const textFiles=[];
...
for(const [theme,baseFile] of Object.entries(themeAssets)){
  if(!experience.includes(`assets/themes/${baseFile}`)||!experienceCss.includes(`assets/themes/${baseFile}`))fail.push(`${theme} self-contained base scene missing from deployed theme runtime`);
  if(!experience.includes(`theme-details/${theme}.svg`)||!experienceCss.includes(`theme-details/${theme}.svg`))fail.push(`${theme} detail layer missing from deployed theme runtime`);
  if(!experience.includes(`theme-icons/${theme}.svg`))fail.push(`${theme} icon sprite missing from deployed theme runtime`);
}
if(experience.includes('theme-composite/'))fail.push('deployed experience controller still depends on nested-resource composite SVGs');
if(experienceCss.includes('content:var(--mf-theme-name)'))fail.push('deployed theme CSS still renders theme-name hero badges');
for(const token of ['body[data-screen="settings"] .main','var(--mf-theme-detail),var(--mf-theme-art)','data-theme="dark"'])if(!experienceCss.includes(token))fail.push(`deployed immersive/light-dark theme contract missing: ${token}`);
...
console.log(`Production audit passed for MilkFlow ${version}: canonical assets, self-contained theme scenes/detail layers, independent components, light/dark contract, service worker, data key, and notification contract are aligned.`);
