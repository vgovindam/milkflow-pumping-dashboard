/* SVG fallbacks for the three painted worlds.
 *
 * These are NOT what ships on a phone. Each world's real artwork is a painted WebP plate, and
 * its dark version is produced by scripts/generate-dark-plates.mjs, which does an actual
 * colour grade. These SVGs exist so a theme still has a scene if a plate ever fails to load,
 * and the dark tint below is only good enough for that job - which is exactly why it must not
 * be allowed near the real dark mode again. It is how three different worlds became the same
 * grey murk.
 */
