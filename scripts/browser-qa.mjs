...
try{
  ...
  for(const theme of themes){
    ...
    for(const route of routes){
      ...
      for(const mode of ['light','dark']){
        ...
        if(route==='mom-home'&&art&&!m.momHeroBg.includes(art))failures.push(`${theme}/${mode}/${route}: selected world is missing from Mom hero (${art})`);
        ...
        if(art&&momRoutes.has(route)&&route!=='mom-home'&&!m.pageHeadArt.includes(art))failures.push(`${theme}/${mode}/${route}: Mom page artwork layer is not themed with ${art}`);
        ...
        if(route==='settings'&&theme!=='clean'&&(m.themeCards!==4||m.selected!==1||m.settingsShortcuts!==4||!m.settingsMotto))failures.push(`${theme}/${mode}/${route}: storybook Settings structure invalid cards=${m.themeCards} selected=${m.selected} shortcuts=${m.settingsShortcuts} motto=${m.settingsMotto}`);
  ...
  if(failures.length){console.error(`Browser QA failed:\n- ${failures.join('\n- ')}`);process.exitCode=1;}else console.log('Browser QA passed 190 views: detailed theme worlds on Mom and Baby, no hero theme labels, independent component art, Settings structure, navigation and light/dark surfaces are rendered from the production build.');
