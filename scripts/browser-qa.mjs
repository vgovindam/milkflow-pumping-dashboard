...
try{
  ...
  const expectedDetails={safari:'theme-details/safari.svg',butterfly:'theme-details/butterfly.svg',princess:'theme-details/princess.svg',unicorn:'theme-details/unicorn.svg'};
  ...
  for(const theme of themes){
    ...
    for(const route of routes){
      ...
      for(const mode of ['light','dark']){
        ...
        if(route==='set-appearance'&&(m.themeCards!==4||m.selected!==1||m.previewLoaded!==4))failures.push(`${theme}/${mode}/${route}: theme selector invalid cards=${m.themeCards} selected=${m.selected} previews=${m.previewLoaded}`);
        if(route==='settings'&&theme!=='clean'&&(m.themeCards!==4||m.selected!==1||m.previewLoaded!==4||m.settingsShortcuts!==4||!m.settingsMotto))failures.push(`${theme}/${mode}/${route}: storybook Settings invalid cards=${m.themeCards} selected=${m.selected} previews=${m.previewLoaded} shortcuts=${m.settingsShortcuts} motto=${m.settingsMotto}`);
  ...
  if(failures.length){console.error(`Browser QA failed:\n- ${failures.join('\n- ')}`);process.exitCode=1;}else console.log('Browser QA passed 190 views: renderable theme scenes, independent detail/icon art, Settings previews, navigation and light/dark surfaces are verified from production dist.');
